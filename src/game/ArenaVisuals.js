...
import { arenaPresetFor, visualQualityFor } from './visualPresets.js';
...
function drawPattern(scene, theme, quality) {
  ...
  if (theme.pattern === 'grid') {
    ...
    g.lineStyle(2, theme.secondary, 0.22).strokeCircle(640, 360, 245).strokeCircle(640, 360, 410);
    return;
  }
  if (theme.pattern === 'city') {
    g.lineStyle(1, theme.primary, 0.13 * quality.decorationAlpha);
    for (let y = 120; y <= 620; y += 100) g.lineBetween(60, y, 1220, y);
    for (let x = 115; x <= 1180; x += 118) {
      const h = 36 + (x % 4) * 14;
      g.fillStyle(theme.ambient, 0.34).fillRect(x - 25, 70, 50, h).fillRect(x - 20, 650 - h, 40, h);
      g.lineStyle(2, x % 2 ? theme.primary : theme.secondary, 0.28).lineBetween(x, 70, x, 70 + h).lineBetween(x, 650 - h, x, 650);
    }
    g.lineStyle(2, theme.secondary, 0.18).strokeRect(180, 170, 920, 380);
    return;
  }
  if (theme.pattern === 'space') {
    g.lineStyle(2, theme.primary, 0.13 * quality.decorationAlpha).strokeEllipse(640, 360, 820, 390).strokeEllipse(640, 360, 520, 520);
    g.lineStyle(1, theme.secondary, 0.12).strokeCircle(640, 360, 112);
    for (let i = 0; i < 7; i++) {
      const a = i * Math.PI * 2 / 7;
      g.fillStyle(i % 2 ? theme.primary : theme.secondary, 0.22).fillCircle(640 + Math.cos(a) * 440, 360 + Math.sin(a) * 230, 4 + (i % 3));
    }
    return;
  }
  ...
  for (const [x1,y1,x2,y2,x3,y3] of cracks) { g.lineBetween(x1,y1,x2,y2); g.lineBetween(x2,y2,x3,y3); }
  g.lineStyle(4, theme.secondary, 0.18).strokeCircle(640, 360, 125);
}
...
function makeAmbient(scene, theme, quality) {
  for (let i = 0; i < quality.ambientCount; i++) {
    ...
    if (quality.id !== 'REDUCED' && i % 3 === 0) scene.tweens.add({ targets: dot, alpha: 0.06, duration: 1100 + (i % 6) * 250, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
  }
}
...
export function drawArenaStorm(scene, value) {
  ...
  scene.stormGlow.lineStyle(thickness + 28, theme.danger, 0.055 + value / 1700).strokeCircle(640, 360, radius);
  ...
  scene.stormRing.lineStyle(1, 0xffffff, 0.28).strokeCircle(640, 360, radius + thickness / 2);
}

export function destroyArenaVisuals(scene) {
